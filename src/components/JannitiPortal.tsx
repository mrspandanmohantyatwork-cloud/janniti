...
import { CivicUpdate, Language, User, CivicFeedback } from '../types';
...
import { transcribeVoiceWithGemini, GeminiVoiceResult } from '../utils/geminiApi';
import { ImageLightboxModal, LightboxImageData } from './ImageLightboxModal';
import { MapPickerModal, MapLocation } from './MapPickerModal';
import {
  Mic,
  Camera,
  Send,
...
export const JannitiPortal: React.FC<JannitiPortalProps> = ({
  ...
  const [selectedLocation, setSelectedLocation] = useState(
    user?.address || user?.ward || 'Patia, Bhubaneswar'
  );
  ...
  // Gemini Voice-to-Text state
  const [isTranscribingVoice, setIsTranscribingVoice] = useState(false);
  const [geminiVoiceResult, setGeminiVoiceResult] = useState<GeminiVoiceResult | null>(null);

  // Transcribe recorded or attached audio using Gemini 2.5 Flash
  const handleTranscribeAudio = async (blob: Blob) => {
    setIsTranscribingVoice(true);
    try {
      const result = await transcribeVoiceWithGemini(blob);
      setGeminiVoiceResult(result);

      // Auto-populate description if empty or citizen speaks a longer complaint
      if (result.transcription) {
        setDescription((prev) => {
          if (!prev.trim()) return result.transcription;
          return prev + '\n\n[Voice Note Transcription]: ' + result.transcription;
        });
      }

      // Auto-detect and suggest civic category if recognized
      if (result.category) {
      ...
      // Auto-suggest detected location if specific
      if (
        result.detectedLocation &&
        !result.detectedLocation.includes('Civic Zone') &&
        result.detectedLocation.length > 3
      ) {
        setSelectedLocation(result.detectedLocation);
      }
    } catch (err) {
  ...
  useEffect(() => {
    if (user?.address || user?.ward) {
      setSelectedLocation(user.address || user.ward || 'Patia, Bhubaneswar');
    }
  }, [user]);
  ...
  const handleDetectGpsLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        setSelectedLocation(`Location (${latitude.toFixed(4)}° N, ${longitude.toFixed(4)}° E)`);
      },
      (err) => {
  ...
  const handleSelectMapLocation = (loc: MapLocation) => {
    ...
    try {
      if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
        ...
        mediaRecorder.onstop = () => {
          ...
          // Automatically transcribe voice to text using Gemini
  ...
  const stopRecording = () => {
    ...
    if (mediaRecorderRef.current && mediaRecorderRef.current.state !== 'inactive') {
      ...
      handleTranscribeAudio(dummyBlob);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const finalDescription = description.trim() || 'Citizen civic development request submitted.';
    ...
    const finalLocation = normalizeExactLocation(selectedLocation.trim() || 'Patia, Bhubaneswar');
    let finalLat = locationCoords.lat;
    let finalLng = locationCoords.lng;
    if (finalLocation.includes('Hotel Num Num') || finalLocation.includes('Acharya Vihar')) {
      finalLat = 20.3015;
      finalLng = 85.8312;
    } else if (finalLocation.includes('Vishal Mega Mart') || finalLocation.includes('Jaydev Vihar')) {
      finalLat = 20.3003;
      finalLng = 85.8242;
    } else if (finalLocation.includes('Kalinga Stadium')) {
      finalLat = 20.3039;
      finalLng = 85.8202;
    }
    ...
    const newUpdate: CivicUpdate = {
      ...
      slaDeadline: isEmergency ? '24-48 working hours' : undefined,
    };
  ...
  const filteredUpdates =
    selectedFilterCategory === 'All'
      ? updates
      : selectedFilterCategory === 'Emergency'
      ? updates.filter((u) => Boolean(u.isEmergency || u.slaDeadline))
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
        ...
        <div className="lg:col-span-5 flex flex-col bg-[var(--card-bg)] backdrop-blur-xl border border-[var(--border-color)] rounded-3xl p-5 sm:p-6 shadow-[0_15px_35px_var(--shadow-color)] h-[620px] overflow-hidden">
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-3 mb-2 no-scrollbar text-xs">
            ...
            {['All', 'Emergency', 'Photos', 'Infrastructure', 'Safety', 'Public Health', 'Transit', 'Waste Management'].map((cat) => (
              <button
                key={cat}
              ...
              >
                {cat === 'Emergency' && <AlertOctagon className="w-3 h-3 text-rose-400" />}
                {cat === 'Photos' && <Camera className="w-3 h-3" />}
                <span>{cat === 'Emergency' ? '🚨 Emergency (24-48h)' : cat === 'Photos' ? 'With Photos' : cat}</span>
              </button>
            ))}
          </div>
            ...
            {filteredUpdates.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center p-6 text-[var(--text-muted)]">
                ...
                <p className="text-xs max-w-xs text-[var(--text-muted)] leading-relaxed">
                  {selectedFilterCategory === 'Photos'
                    ...
                    : 'Be the first citizen in your area to report an issue or suggest a development project using the form on the right.'}
                </p>
              </div>
            ) : (
              filteredUpdates.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className={`bg-[var(--item-bg)] hover:bg-[var(--item-hover)] p-4 rounded-2xl border transition-all duration-200 ${
                      item.isEmergency
                        ? 'border-rose-500/50 border-l-4 border-l-rose-500 shadow-[0_0_15px_rgba(244,63,94,0.15)]'
                        : 'border-[var(--border-color)] border-l-4 border-l-purple-500'
                    }`}
                  >
                    {/* Emergency Hazard SLA Ribbon */}
                    {item.isEmergency && (
                      <div className="mb-2.5 px-2.5 py-1.5 rounded-xl bg-gradient-to-r from-rose-500/20 via-amber-500/15 to-rose-500/5 border border-rose-500/30 flex items-center justify-between">
                        <div className="flex items-center gap-1.5 text-rose-400 text-[11px] font-bold">
                          <span className="relative flex h-2 w-2">
                            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-rose-400 opacity-75"></span>
                            <span className="relative inline-flex rounded-full h-2 w-2 bg-rose-500"></span>
                          </span>
                          <span className="uppercase tracking-wider">🚨 Emergency Issue</span>
                        </div>
                        <span className="text-[10px] font-semibold text-amber-300 flex items-center gap-1 bg-black/30 px-2 py-0.5 rounded-md border border-amber-500/20">
                          <Clock className="w-3 h-3 text-amber-400" />
                          {item.status === 'resolved' ? '✅ Resolved in SLA' : '⏱️ 24–48h SLA'}
                        </span>
                      </div>
                    )}

                    <div className="flex items-center justify-between mb-1.5">
                      <div className="flex items-center gap-1.5 text-xs font-bold text-sky-400 truncate max-w-[72%]">
                        ...
                        <span className="text-[var(--text)] font-semibold shrink-0">{item.category}</span>
                      </div>
                      <span
                    ...
                    {/* Resolved Status Indicator (Feedback option removed from Live Civic Updates) */}
                    {item.status === 'resolved' && (
                      <div className="mb-3 px-3 py-2 rounded-xl bg-emerald-950/20 dark:bg-emerald-950/30 border border-emerald-500/25 flex items-center justify-between">
                        <span className="text-xs text-emerald-400 font-semibold flex items-center gap-1.5">
                          <CheckCircle2 className="w-4 h-4 text-emerald-400" />
                          <span>Civic Issue Resolved by Authority</span>
                        </span>
        ...
        <div id="citizen-petition-form" className="lg:col-span-7 flex flex-col bg-[var(--card-bg)] backdrop-blur-xl border border-[var(--border-color)] rounded-3xl p-6 sm:p-8 shadow-[0_15px_35px_var(--shadow-color)] relative">
          <div className="mb-6">
              ...
              {audioUrl && (
                <div className="flex flex-col gap-2 w-full sm:w-auto">
                  <div className="flex items-center gap-2 bg-sky-500/15 border border-sky-500/30 px-3 py-2 rounded-xl text-xs text-sky-300">
                    ...
                    {/* Gemini Transcribe Action */}
                    {audioBlob && (
                      <button
                        type="button"
                        disabled={isTranscribingVoice}
                        onClick={() => handleTranscribeAudio(audioBlob)}
                        className="ml-2 px-2.5 py-1 rounded-lg bg-sky-500 hover:bg-sky-400 text-slate-950 font-bold text-[11px] flex items-center gap-1 transition-all cursor-pointer shadow-xs disabled:opacity-50"
                        title="Transcribe speech to text using Gemini 2.5 Flash"
                      >
                        <Sparkles className="w-3 h-3" />
                        <span>{isTranscribingVoice ? 'Gemini transcribing...' : 'Convert to Text with Gemini'}</span>
                      </button>
                    )}
                    ...
                    <button
                      ...
                      onClick={() => {
                        setAudioUrl(null);
                        setAudioBlob(null);
                        setGeminiVoiceResult(null);
                      }}
                      className="text-rose-400 hover:text-rose-300 ml-auto cursor-pointer p-1"
                  ...
                  {/* Live Gemini Speech-to-Text Status or Result Banner */}
                  {isTranscribingVoice && (
                    <div className="p-2.5 rounded-xl bg-purple-500/10 border border-purple-500/30 text-purple-300 text-xs flex items-center gap-2 animate-pulse">
                      <Sparkles className="w-4 h-4 text-purple-400 shrink-0" />
                      <span>Gemini 2.5 Flash is analyzing your audio recording and converting speech to text...</span>
                    </div>
                  )}

                  {geminiVoiceResult && !isTranscribingVoice && (
                    <div className="p-3 rounded-xl bg-sky-500/10 border border-sky-500/30 space-y-1.5 text-xs text-[var(--text)]">
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex items-center gap-1.5 font-bold text-sky-400 text-[11px] uppercase tracking-wider">
                          <Sparkles className="w-3.5 h-3.5 text-sky-400" />
                          <span>Gemini Voice-to-Text Transcription</span>
                        </div>
                        {geminiVoiceResult.urgency && (
                          <span className={`px-2 py-0.5 rounded-full text-[10px] font-black uppercase ${
                            geminiVoiceResult.urgency === 'CRITICAL' ? 'bg-rose-500/20 text-rose-400' :
                            geminiVoiceResult.urgency === 'HIGH' ? 'bg-amber-500/20 text-amber-400' :
                            'bg-sky-500/20 text-sky-400'
                          }`}>
                            {geminiVoiceResult.urgency} Urgency
                          </span>
                        )}
                      </div>
                      <p className="text-[11px] text-[var(--text)] italic bg-black/20 p-2 rounded-lg border border-[var(--border-color)]">
                        &ldquo;{geminiVoiceResult.transcription}&rdquo;
                      </p>
                      <div className="flex flex-wrap items-center gap-2 text-[10px] text-[var(--text-muted)]">
                        {geminiVoiceResult.category && (
                          <span className="px-1.5 py-0.5 rounded bg-sky-500/20 text-sky-300 font-semibold">
                            Category: {geminiVoiceResult.category}
                          </span>
                        )}
                        {geminiVoiceResult.detectedLocation && (
                          <span className="px-1.5 py-0.5 rounded bg-purple-500/20 text-purple-300 font-semibold">
                            Location: {geminiVoiceResult.detectedLocation}
                          </span>
                        )}
                        <span className="text-emerald-400 ml-auto font-bold flex items-center gap-1">
                          <CheckCircle2 className="w-3 h-3" /> Auto-populated in grievance
                        </span>
                      </div>
                    </div>
                  )}
          ...
          <form onSubmit={handleSubmit} className="flex flex-col flex-1">
              ...
              <div>
                <div className="flex items-center justify-between text-xs font-semibold text-[var(--text-muted)] mb-1.5">
                  ...
                  <button
                    ...
                    title="Open precision map picker"
                  >
                ...
                {/* Compressed Location Input Bar */}
                <div className="relative flex items-center">
                  <div className="absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none text-sky-400">
                  ...
                  <input
                    ...
                    onBlur={() => {
                      if (selectedLocation.trim()) {
                        const norm = normalizeExactLocation(selectedLocation);
                        if (norm !== selectedLocation) {
                          setSelectedLocation(norm);
                          if (norm.includes('Hotel Num Num') || norm.includes('Acharya Vihar')) {
                            setLocationCoords({ lat: 20.3015, lng: 85.8312 });
                          } else if (norm.includes('Vishal Mega Mart') || norm.includes('Jaydev Vihar')) {
                            setLocationCoords({ lat: 20.3003, lng: 85.8242 });
                          } else if (norm.includes('Kalinga Stadium')) {
                            setLocationCoords({ lat: 20.3039, lng: 85.8202 });
                          }
                        }
                      }
                    }}
                    placeholder="Enter or pin exact location..."
                    className="w-full pl-9 pr-24 py-2 text-xs sm:text-sm rounded-xl bg-[var(--input-bg)] border border-[var(--border-color)] text-[var(--text)] outline-none focus:border-sky-400 font-medium transition-all"
                  />
                  <div className="absolute right-1.5 top-1/2 -translate-y-1/2 flex items-center gap-1">
                    <button
                      ...
                      className="px-2 py-1 rounded-lg bg-[var(--card-bg)] hover:bg-sky-500/20 text-[10px] font-semibold text-sky-400 border border-[var(--border-color)] flex items-center gap-1 cursor-pointer transition-colors"
                    ...
                    <button
                      type="button"
                      onClick={() => setIsMapModalOpen(true)}
                      className="px-2.5 py-1 rounded-lg bg-sky-500 hover:bg-sky-600 text-[10px] font-bold text-white shadow-sm flex items-center gap-1 cursor-pointer transition-colors"
                      title="Pin on Mapbox / OpenStreetMap"
                    >
            ...
            {/* Emergency 24-48 Working Hours SLA Fast-Track Option */}
            <div
              onClick={() => setIsEmergency(!isEmergency)}
              className={`mb-4 p-3.5 rounded-2xl border transition-all cursor-pointer flex items-start gap-3 select-none ${
                isEmergency
                  ? 'bg-rose-500/15 border-rose-500 shadow-[0_0_20px_rgba(244,63,94,0.25)]'
                  : 'bg-[var(--item-bg)] border-[var(--border-color)] hover:border-rose-500/40'
              }`}
            >
              <div
                className={`mt-0.5 w-5 h-5 rounded-md border flex items-center justify-center shrink-0 transition-colors ${
                  isEmergency ? 'bg-rose-500 border-rose-400 text-white' : 'border-slate-500 bg-transparent'
                }`}
              >
                {isEmergency && <Check className="w-3.5 h-3.5 stroke-[3]" />}
              </div>
              <div className="flex-1">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-xs sm:text-sm font-extrabold text-[var(--text)] flex items-center gap-1.5">
                    <AlertOctagon className={`w-4 h-4 ${isEmergency ? 'text-rose-400' : 'text-slate-400'}`} />
                    Mark as Emergency Issue (24–48 Working Hours SLA)
                  </span>
                ...
                <p className="text-[11px] text-[var(--text-muted)] mt-0.5 leading-relaxed">
                  Select if this issue presents an urgent public safety, road hazard, or health risk (live wires, broken water main, cave-in, open manhole). The municipal fast-track squad will resolve it within <strong>24 to 48 working hours</strong>.
                </p>
                {isEmergency && (
                  <div className="mt-2 text-[10px] font-bold text-amber-300 flex items-center gap-1.5 bg-amber-500/15 border border-amber-500/30 px-2.5 py-1 rounded-lg">
                    <Clock className="w-3.5 h-3.5 text-amber-400 shrink-0" />
                    <span>Rapid Emergency Protocol Activated &bull; Target Resolution: 24–48 Working Hours</span>
                  </div>
                )}
              </div>
            </div>
            ...
            {submittedReceipt && (
              <div className="mb-4 p-3.5 rounded-2xl bg-emerald-500/15 border border-emerald-500/40 text-emerald-300 flex flex-col gap-2 text-xs animate-in fade-in">
                ...
                {submittedReceipt.isEmergency && (
                  <div className="pt-2 border-t border-emerald-500/20 text-[11px] font-bold text-amber-300 flex items-center gap-1.5">
                    <AlertOctagon className="w-3.5 h-3.5 text-rose-400 shrink-0" />
                    <span>Emergency Fast-Track Assigned: Targeted Resolution within 24–48 Working Hours</span>
                  </div>
                )}
              </div>
            )}
